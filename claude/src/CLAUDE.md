...
- **BLOCKING**: Before proposing ANY change to existing code (feature, fix, refactor), READ `references/architectural-consolidation.md`. The implementation plan you present MUST include the architectural map: existing concepts identified, chosen outcome (extend / redraw / introduce-new-with-named-relationship / surface), and named relationships to neighbors. Consolidate over append. No exceptions.
- **BLOCKING**: Before starting ANY significant change in an existing project, READ `references/pre-work-checklist.md` and ensure a clean git save point. If the working tree is dirty, STOP and ASK the user — NEVER stash, commit, or discard uncommitted changes autonomously. No exceptions.
- **BLOCKING**: When a dirty working tree must be resolved (detected via `pre-work-checklist.md` or otherwise), READ `references/uncommitted-changes-triage.md` and DELEGATE the triage to a sub-agent. NEVER investigate uncommitted changes in the active session — it pollutes the context of the task you were asked to do. Surface a structured report with origin, session IDs, dates, and intent before asking the user what to do. No exceptions.
